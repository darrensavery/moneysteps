...
import { handleRegisterDeviceToken, handleUnregisterDeviceToken, handleGetPendingCount } from './routes/push.js';
...
async function route(request: Request, env: Env, ctx: ExecutionContext, method: string, path: string): Promise<Response> {
  ...
  if (path === '/api/push/pending-count' && method === 'GET') return withAuth(request, auth, env, ctx, handleGetPendingCount);
